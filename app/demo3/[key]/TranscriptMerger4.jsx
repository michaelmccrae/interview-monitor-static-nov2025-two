...
function hasAnnotations(turn) {
  // Check specifically for non-empty errors here too if needed,
  // but usually the main render loop filter is sufficient.
  // We'll leave this broader to catch edge cases, or refine it if you want to hide turns entirely.
  const hasValidError =
...
function highlightTerms(
  ...
  // Filter errors here as well to prevent highlighting empty strings (which would highlight everything)
  ...
  return segments.map((seg, i) => {
    ...
    if (seg.type === "lookup") {
      ...
      // Determine style classes based on active state and view mode
      let buttonClasses = "";

      if (isActive) {
        buttonClasses = "bg-blue-500/20 border-blue-400 text-blue-200";
      } else if (useWhiteButtons) {
        // DETAILED VIEW (White background buttons)
        buttonClasses =
          "bg-white border-white/50 text-zinc-900 hover:bg-zinc-100";
      } else {
        // CONDENSED VIEW (Default dark/transparent buttons)
          ...
          {items.map((item) => {
            ...
            return (
              <div key={item.ID} className="space-y-2">
                ...
                <div className="pl-6">
                  {/* Errors List - Only render if validErrors exist */}
